      ...
      mysqlDB.getConnection(function(err, connection){
        if(err){
          ...
          connection.query(query, [roomname], function(err, rows, fields){
              if(err){
                console.log('채팅 메인 화면 쿼리 에러'+err);
              }else{    
                var params = {
      ...
      mysqlDB.getConnection(function(err, connection){
        if(err){
          ...
          connection.query(query, [req.session.userId, req.session.userId], function(err, rows, fields){
              if(err){
                console.log('채팅 대화 목록 쿼리 에러'+err);
              }else{      
                ...
                connection.query(subQuery, [req.session.userId], function(err, subRows, fields){
                    if(err){
                      console.log('채팅 대화 목록 서브 쿼리 에러'+err);
                    }else{
                      var params = {
        ...
        if(err){
          ...
          connection.query(query, [socket.id], function(err, rows, fields){
              if(err){
                ...
                var idx = room.indexOf(sessionUserId);
                var uid = room.substring(0,idx);
                if(uid === ''){
                  setChatList(sessionUserId, room);
                }else{
                  setChatList(uid, room);
                }
                connection.release();
              }
          });   
  ...
  mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [socketId, sessionEmail], function(err, rows, fields){
          if(err){
            console.log('소켓아이디 DB 입력 에러'+err);
          }else{      
  ...
  mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [userId, message, roomName, timeStamp_chat], function(err, rows, fields){
          if(err){
            console.log('채팅방 대화내용 DB 입력 에러'+err);
          }else{       
            connection.release();
          }
      });   
   ...
   mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [userId, roomName, userId, roomName], function(err, rows, fields){
          if(err){
            console.log('채팅방 참가 유저 테이블 입력 에러'+err);
          }else{       
            connection.release();
          }
      });   
  ...
  mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [sessionUserId, userId_w, roomName, now, roomStatus, roomName], function(err, rows, fields){
          if(err){
            console.log('채팅방 목록 데이터 생성 에러'+err);
          }else{      
            connection.release();
          }
      });   
  ...
  mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [userId_w, roomName, userId_w, roomName], function(err, rows, fields){
          if(err){
            console.log('대화 상대 참가자 목록 테이블 입력 에러'+err);
          }else{       
            connection.release();
          }
      });   
...
function exitRoom(roomName, userId){  
  mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [roomName], function(err, rows, fields){
          if(err){
            console.log('채팅방 대화 기록 삭제 에러'+err);
          }else{                        
  ...
  mysqlDB.getConnection(function(err, connection){
    if(err){
      ...
      connection.query(query, [roomName, userId], function(err, rows, fields){
          if(err){
            console.log('채팅방 참가자 기록 삭제 에러'+err);
          }else{                        
